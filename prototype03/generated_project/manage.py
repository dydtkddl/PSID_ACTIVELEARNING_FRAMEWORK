...
def main():
    ...
    df = pd.read_csv(MOF_DB_DIR / csv_name)
    ...
    # CIF 파일 리스트 확인
    cif_path = Path(cfg['RASPA_DIR']) / 'share' / 'raspa' / 'structures' / 'cif'
    ...
    lp.to_sql('low_pressure_gcmc', conn, if_exists='replace', index=False)
    al.to_sql('active_learning_gcmc', conn, if_exists='replace', index=False)
    conn.close()
